...
class PlayGame():
    def __init__(self, relevant_root):
        ...
        self.cards = self._generate_game_list(self._pairs_to_find, self._sudden_deaths)
...
#                self.buttons[input_button_number].configure(text = self.cards[input_button_number])
                ...
                self._open_cards.remove(input_button_number) # poistetaan se sieltä
#                print(f" päivitetty avoimet kortit lista: {self._open_cards}")
            else: # if you press a third card, nothing happens (except error message)
                self.info_label.configure(text = "2 cards are open - close at least 1 to continue")
...
#                if self.cards[input_button_number] == "sudden": # works
                if self.cards[input_button_number][1] == "sudden":
                    ...
                    if self._lives > 0: # if lives are still left
                        self.lives_label.configure(text = (f"{self._lives} lives left"))
                        self.info_label.configure(text = "sudden death: now you lost a life")
                    if self._lives == 0:
                        self.lives_label.configure(text = (f"{self._lives} lives left"))
...
#                    self.buttons[input_button_number].configure(
                    ...
                    if len(self._open_cards) == 2: # and now there are two cards open
                        if self.cards[self._open_cards[0]] == self.cards[self._open_cards[1]]:
                            ...
                            self.buttons[self._open_cards[0]].configure(state = "disabled")
                            self.buttons[self._open_cards[1]].configure(state = "disabled")
                            self._points += 20
                            self.points_label.configure(text = f"{self._points} points")
                            self._pairs_found += 1
                            self._open_cards = []
                            if self._pairs_found == self._pairs_to_find: # you find all the pairs
                                self._points += self._pairs_to_find*20*2 # get extra points
                                self.points_label.configure(text = f"{self._points} points")
                                self._game_over("CONGRATULATIONS! LEVEL CLEARED")
