...
class PlayerDatabaseInteraction():
    ...
    def get_all_players_names(self):
        ...
        """
        self._player_database.create_player_database_connection()

        all_players_names = self._player_database.player_db.execute(
    ...
    def create_new_player(self, given_name):
        ...
        self._player_database.player_db.execute("""INSERT INTO
            Players (name, points, lives, maxlevel)
            VALUES(?, 0, 5, 3)""", [given_name])
    ...
    def get_one_player_stats(self, given_name):
        ...
        if self.check_player_exists(given_name) is True:
            answer = self._player_database.player_db.execute("""SELECT *
                                    FROM Players
    ...
    def _get_player_max_level(self, given_name):
        """Method retrieves and returns from the database the maximum level
        ...
        """
        self._player_database.create_player_database_connection()
        player_all_stats = self.get_one_player_stats(given_name)
    ...
    def _get_current_game_specs(self):
        ...
        self._player_database.create_player_database_connection()
        raw_answer = self._player_database.player_db.execute("""SELECT *
    ...
    def update_player_stats(self, new_stats):
        ...
        self._player_database.player_db.execute("""UPDATE Players
                        SET points = ?,
                            lives = ?,
                            maxlevel = ?
                        WHERE name = ?
    ...
    def _set_default_player_data(self):
        ...
        if self.check_player_exists(default_player_name) is True:
            self._player_database.player_db.execute("""UPDATE Players
                        SET points = 0,
                            lives = 5,
                            maxlevel = 3
            ...
            self._player_database.player_db.execute(
                """INSERT INTO Players (name, points, lives, maxlevel)
                VALUES("NoName", 0, 5, 3)""")

    def delete_player(self, given_name):
        """ Method removes a player with the given name from the database.
        """
        self._player_database.create_player_database_connection()

        player_to_be_removed = given_name

        if self.check_player_exists(player_to_be_removed) is True:
            self._player_database.player_db.execute("""DELETE
                        FROM Players
